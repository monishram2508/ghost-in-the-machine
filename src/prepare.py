...
def extract_topics(path_chunked,path_topics):
    topics=Counter()
    path_topics.mkdir(parents=True,exist_ok=True)
    prev_base="start"
    for file in sorted(path_chunked.glob("*.txt")):
        base=file.stem.split("_")
        text=file.read_text(encoding="utf-8")
        base="_".join(base[:-2])
        if base==prev_base or prev_base=="start":
            topics=tokenize(text,topics)
            #tokenize will take in the text and the topics list and append to it, the top 10 high frequency words
            prev_base=base
        else:
            topwords=[]
            for word,count in topics.most_common(10): 
                topwords.append(word)
            write_path=path_topics/f"{prev_base}_topics.txt"
            #topics is a list, we cant write a list into a text file
            write_path.write_text('\n'.join(topwords),encoding="utf-8")
            topics=Counter()
            topics=tokenize(text,topics)
            prev_base=base
    if topics:
        ...
        for word,count in topics.most_common(10):
            topwords.append(word)
        write_path=path_topics/f"{base}_topics.txt"
        write_path.write_text('\n'.join(topwords),encoding="utf-8")
...
if __name__=="__main__":
    read(path_raw)
    chunk(path_cleaned)
    extract_topics(path_chunked,path_topics)